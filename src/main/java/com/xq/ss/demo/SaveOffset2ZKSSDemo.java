 ...
 *
 */
public class SaveOffset2ZKSSDemo implements Serializable {
	private static final long serialVersionUID = 1L;

	public static void main(String[] args) throws Exception {
//		Logger.getLogger("org.apache.spark").setLevel(Level.INFO);
		SparkConf conf = new SparkConf().setAppName("TestOffset2ZK");
...
//		kafkaParameters.put("metadata.broker.list", "pg253liulu:9092");
		...
		kafkaParameters.put("group.id", "xq-test-pg-zk");
...
//		Collection<String> topics = Arrays.asList("kafka123");
		Map<TopicPartition, Long> offsets = ZKTools.getInstance().getOffsets(topics, kafkaParameters);
...
//		list.add(new Tuple3<String, Integer, Long>("kafka123", 0, 39641982L));
		...
		JavaInputDStream<ConsumerRecord<String, String>> dStream = KafkaUtils.createDirectStream(
				...
				ConsumerStrategies.<String, String>Assign(offsets.keySet(), kafkaParameters, offsets));
		/*JavaInputDStream<ConsumerRecord<String, String>> dStream = KafkaUtils.createDirectStream(jsc,
				LocationStrategies.PreferConsistent(),
				ConsumerStrategies.<String, String>Subscribe(topics, kafkaParameters,offsets));*/
		
		// 得到rdd各个分区对应的offset, 并保存在offsetRanges?
//        final AtomicReference<OffsetRange[]> offsetRanges = new AtomicReference<OffsetRange[]>();
        
		/*JavaDStream<ConsumerRecord<String, String>> transform = lines.transform(new Function2<JavaRDD<ConsumerRecord<String, String>>, Time,JavaRDD<ConsumerRecord<String, String>>>() {
			private static final long serialVersionUID = 1L;

			@Override
			public JavaRDD<ConsumerRecord<String, String>> call(JavaRDD<ConsumerRecord<String, String>> rdd, Time time)
					throws Exception {
				...
				for (OffsetRange o : offsets) {
					...
					o.topicPartition();
					o.count();
					System.out.println(o.fromOffset()+";"+o.untilOffset()+";"+o.count()+";"+o.partition()+";"+o.topic());
				}
				return rdd;
			}
		});
